...
from policy.temporal_yopo_network import TemporalYopoNetwork as TemporalYopoNetworkV1
from policy.temporal_yopo_network_v2 import TemporalYopoNetwork as TemporalYopoNetworkV2
...
class YopoNet:
    def __init__(self, config, weight):
        ...
        self.depth_history = deque(maxlen=self.history_length)
        ...
        checkpoint = torch.load(weight, map_location=self.device, weights_only=False)
        state_dict = checkpoint["model"] if "model" in checkpoint else checkpoint
        network_class = (
            TemporalYopoNetworkV2
            if any(key.startswith("temporal_fusion.") for key in state_dict)
            else TemporalYopoNetworkV1
        )
        self.policy = network_class()
    ...
    def callback_temporal_depth(self, data):
        ...
        if len(self.depth_history) < self.history_length:
            rospy.loginfo_throttle(1.0, f"Temporal YOPO history {len(self.depth_history)}/{self.history_length}")
            return

        depth = np.stack(self.depth_history, axis=0)[None, ...]
        relative_poses = []
    ...
    def warm_up(self):
        ...
        relative_pose = torch.zeros((1, int(cfg["relative_pose_dim"])), dtype=torch.float32, device=self.device)
...
def parser():
    ...
    parser.add_argument("--weight", default="", help="temporal YOPO checkpoint")
    return parser
...
if __name__ == "__main__":
    ...
    weight = args.weight or (base_dir + "/saved_temporal/temporal_dynamic_independent_low_alt_20260730/best.pth")
