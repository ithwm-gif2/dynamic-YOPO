...
class YopoNet:
    def __init__(self, config, weight):
        ...
        self.history_length = int(cfg["sd_history_length"])
        self.buffer_length = max(self.frame_offsets) + 1
        self.depth_history = deque(maxlen=self.buffer_length)
        self.pose_history = deque(maxlen=self.buffer_length)
        self.last_depth_stamp = None
        self.max_frame_gap = 0.15

        # Load Network
        if self.use_trt:
            raise NotImplementedError("Temporal YOPO TensorRT export is not implemented yet")
        checkpoint = torch.load(
            weight, map_location=self.device, weights_only=False
        )
        state_dict = checkpoint["model"] if "model" in checkpoint else checkpoint
        self.policy = StaticDynamicYopoNetwork()
        ...
        self.depth_sub = rospy.Subscriber(self.config['depth_topic'], Image, self.callback_temporal_depth, queue_size=1, tcp_nodelay=True)
    ...
    def process_odom(self):
        ...
        planning_pos = self.desire_pos if self.plan_from_reference else current_pos
        goal_w = self.goal - planning_pos
    ...
    @torch.inference_mode()
    def callback_temporal_depth(self, data):
        ...
        if (self.last_depth_stamp is not None and
                (stamp <= self.last_depth_stamp or
                 stamp - self.last_depth_stamp > self.max_frame_gap)):
        ...
        self.pose_history.append((camera_position, self.Rotation_wc.copy()))
        if len(self.depth_history) < self.buffer_length:
            rospy.loginfo_throttle(
                1.0,
                f"Static-dynamic YOPO history "
                f"{len(self.depth_history)}/{self.buffer_length}",
            )
            return

        selected_indices = [
            self.buffer_length - 1 - offset for offset in self.frame_offsets
        ]
        depth_frames = list(self.depth_history)
        pose_frames = list(self.pose_history)
        depth = np.stack(
            [depth_frames[index] for index in selected_indices], axis=0
        )[None, ...]
        relative_poses = []
        poses = [pose_frames[index] for index in selected_indices]
        for previous, current in zip(poses[:-1], poses[1:]):
            previous_position, previous_rotation = previous
            current_position, current_rotation = current
            relative_translation = previous_rotation.T @ (current_position - previous_position)
            relative_rotation = previous_rotation.T @ current_rotation
            relative_rotvec = R.from_matrix(relative_rotation).as_rotvec()
            relative_poses.append(np.concatenate((relative_translation, relative_rotvec)))
        relative_pose = np.concatenate(relative_poses).astype(np.float32)[None, :]

        time1 = time.time()
        depth_input = torch.from_numpy(depth).to(self.device, non_blocking=True)
        relative_pose_input = torch.from_numpy(relative_pose).to(self.device, non_blocking=True)
        obs_input = self.state_transform.prepare_input(obs_norm.to(self.device, non_blocking=True))
        time2 = time.time()
        endstate_pred, score_pred = self.policy(depth_input, obs_input, relative_pose_input)
        endstate_pred, score_pred = endstate_pred.cpu().numpy(), score_pred.cpu().numpy()
        time3 = time.time()
    ...
    def warm_up(self):
        depth = torch.zeros((1, self.history_length, self.height, self.width), dtype=torch.float32, device=self.device)
        obs = torch.zeros((1, 9), dtype=torch.float32, device=self.device)
        obs = self.state_transform.prepare_input(obs)
        relative_pose = torch.zeros((1, int(cfg["sd_relative_pose_dim"])), dtype=torch.float32, device=self.device)
        endstate_pred, score_pred = self.policy(depth, obs, relative_pose)
...
def parser():
    ...
    parser.add_argument("--weight", default="", help="static-dynamic YOPO checkpoint")
    return parser
...
if __name__ == "__main__":
    ...
    weight = args.weight or (
        base_dir
        + "/saved_static_dynamic/"
        + "sd_yopo_velocity_fusion_safety_rank_full_20260731/best.pth"
    )
